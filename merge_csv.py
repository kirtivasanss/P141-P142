...
with open('Movie Recomendation\movies.csv', errors = 'replace') as f:
    reader = csv.reader(f)
    data = list(reader)
    all_movies = data[1:]
    headers = data[0]
...
with open("final.csv","a+", errors = 'replace') as f:
    csvwriter = csv.writer(f)
    csvwriter.writerow(headers)

with open('Movie Recomendation\poster.csv', errors = 'replace') as f:
    reader = csv.reader(f)
    data = list(reader)
